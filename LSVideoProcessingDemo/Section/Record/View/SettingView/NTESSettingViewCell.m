...
const NSInteger gMaxBtnCount = 4;
...
- (void)layoutSubviews
{
    ...
    _scroll.frame = CGRectMake(_titleLab.right + 32.0,
                               0,
                               self.width - (_titleLab.right + 32.0) - 16.0,
                               self.height);
    
    _line.frame = CGRectMake(16.0, self.height - 1, self.width - 16.0*2, 1.0);
...
{
    CGFloat width = (_scroll.width - 16.0)/gMaxBtnCount;
    CGFloat height = self.scroll.height;
    
    //滚动视图
    CGFloat contentWidth = ((_btns.count > gMaxBtnCount) ? (gMaxBtnCount * width * 2) : (gMaxBtnCount * width));
